...
type TaxExportResult = GstReport | VatReport | SalesTaxReport | TaxReportSummary;
...
export class TaxExportService {
  ...
  async exportTaxReport(
    ...
    const typedOrders: OrderForExport[] = orders.map(o => ({
      ...
      pricing: (o.pricing as unknown as OrderForExport['pricing']) || { subtotal: 0, taxRate: 0, taxAmount: 0, total: 0 },
      status: o.status,
    ...
    switch (taxSettings.regime) {
      case 'gst_india':
        report = this.buildGstReport(typedOrders, customerMap, taxSettings, from, to);
        break;
      case 'vat_eu':
      case 'vat_uk':
        report = this.buildVatReport(typedOrders, customerMap, taxSettings, from, to);
        break;
      case 'sales_tax_us':
        report = this.buildSalesTaxReport(typedOrders, customerMap, taxSettings, from, to);
        break;
      default:
  ...
  private buildGstReport(
    ...
    let b2cSmall = { taxableValue: 0, cgst: 0, sgst: 0, totalTax: 0 };
    ...
    for (const order of orders) {
      ...
      if (customer?.taxId) {
        ...
        if (!entry) {
          entry = { customerTaxId: customer.taxId, customerName: customer.name, invoices: [] };
        ...
        b2cLargeInvoices.push({
          invoiceNumber: order.invoiceNumber || order.orderNumber,
          invoiceDate: order.createdAt.toISOString().split('T')[0],
          placeOfSupply: settings.gstConfig?.placeOfSupply || settings.regionCode,
          taxableValue: order.pricing.subtotal,
          rate: order.pricing.taxRate,
          igst,
        });
  ...
  private buildVatReport(
    ...
    const ecSalesMap = new Map<string, { customerVatId: string; customerName: string; country: string; totalValue: number }>();

    for (const order of orders) {
      const customer = order.customerId ? customerMap.get(order.customerId) : null;
      const customerName = customer?.name || (order.customerInfo?.name as string) || 'Walk-in';
      const breakdown = order.pricing.taxBreakdown;
      const vatAmount = breakdown?.componentTotals?.['VAT'] || order.pricing.taxAmount;
      ...
      if (settings.regime === 'vat_eu' && customer?.taxId && breakdown?.metadata?.isInterState) {
    ...
    return {
      ...summary,
      domesticSales,
      ecSalesList: ecSalesMap.size > 0 ? Array.from(ecSalesMap.values()) : undefined,
    };
  }
  ...
  private buildSalesTaxReport(
    ...
    const stateMap = new Map<string, { state: string; taxableValue: number; taxAmount: number; invoiceCount: number }>();
  ...
  private buildSummary(
    ...
    const rateMap = new Map<number, { taxableValue: number; taxAmount: number; invoiceCount: number }>();
    ...
    for (const order of orders) {
      ...
      const breakdown = order.pricing.taxBreakdown;
      if (breakdown) {
        for (const [name, amount] of Object.entries(breakdown.componentTotals)) {
  ...
  private convertToCsv(report: TaxExportResult, regime: TaxRegime): string {
    ...
    for (const rs of report.rateSummary) {
      lines.push(`${rs.rate},${rs.taxableValue},${rs.taxAmount},${rs.invoiceCount}`);
    }
    lines.push('');

    // Regime-specific sections
    if (regime === 'gst_india' && 'b2b' in report) {
      const gst = report as GstReport;

      lines.push('B2B Transactions');
      lines.push('Customer GSTIN,Customer Name,Invoice Number,Invoice Date,Taxable Value,Rate (%),CGST,SGST,IGST,Total');
      ...
      if (gst.b2cLarge.length > 0) {
        ...
        lines.push('Invoice Number,Invoice Date,Place of Supply,Taxable Value,Rate (%),IGST');
        for (const inv of gst.b2cLarge) {
          lines.push(`${inv.invoiceNumber},${inv.invoiceDate},${inv.placeOfSupply},${inv.taxableValue},${inv.rate},${inv.igst}`);
        }
        lines.push('');
      }
      ...
      lines.push(`${gst.b2cSmall.taxableValue},${gst.b2cSmall.cgst},${gst.b2cSmall.sgst},${gst.b2cSmall.totalTax}`);
      lines.push('');

      if (gst.hsnSummary.length > 0) {
        lines.push('HSN Summary');
        lines.push('HSN Code,Description,Quantity,Taxable Value,Rate (%),CGST,SGST,IGST,Total Tax');
    ...
    } else if ((regime === 'vat_eu' || regime === 'vat_uk') && 'domesticSales' in report) {
      const vat = report as VatReport;

      lines.push('Domestic Sales');
      lines.push('Invoice Number,Invoice Date,Customer Name,Taxable Value,VAT Rate (%),VAT Amount,Total');
      ...
      if (vat.ecSalesList && vat.ecSalesList.length > 0) {
        ...
        for (const ec of vat.ecSalesList) {
          lines.push(`${ec.customerVatId},${this.csvEscape(ec.customerName)},${ec.country},${ec.totalValue}`);
        }
      }
    } else if (regime === 'sales_tax_us' && 'stateBreakdown' in report) {
      const st = report as SalesTaxReport;

      lines.push('State Breakdown');
      lines.push('State,Taxable Value,Tax Amount,Invoice Count');
      for (const entry of st.stateBreakdown) {
        lines.push(`${entry.state},${entry.taxableValue},${entry.taxAmount},${entry.invoiceCount}`);
      }
    }
