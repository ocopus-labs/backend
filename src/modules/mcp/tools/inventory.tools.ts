...
export function registerInventoryTools(server: McpServer, ctx: McpContext) {
  ...
  if (ctx.hasPermission('inventory:read')) {
    server.tool(
      ...
      'List inventory items with optional filters (category, status, low stock)',
      {
        ...
        status: z
          .string()
          .optional()
          .describe('Filter by status: in_stock, low_stock, out_of_stock'),
      ...
      async (params) => {
        ...
        await ctx.audit('inventory.list', 'inventory', null, {
          filters: params,
        });
        return {
    ...
    server.tool(
      ...
      async ({ itemId }) => {
        const result = await ctx.inventoryService.findByIdOrFail(
          ctx.businessId,
          itemId,
        );
    ...
    server.tool(
      ...
      {},
      async () => {
        const result = await ctx.inventoryService.getLowStockItems(
          ctx.businessId,
        );
    ...
    server.tool(
      ...
      {},
      async () => {
        const result = await ctx.inventoryService.getInventoryStats(
          ctx.businessId,
        );
